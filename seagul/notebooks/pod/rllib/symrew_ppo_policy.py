# This is a modification of https://github.com/ray-project/ray/blob/master/rllib/agents/ppo/ppo_policy.py
# to support symmetric learning
...
from seagul.notebooks.pod.rllib.mirror_fns import (
    mirror_walker_obs,
    mirror_walker_act,
    mirror_human_obs,
    mirror_human_act,
...
class PPOLoss(object):
    def __init__(
        ...
        surrogate_loss = tf.minimum(
            advantages * logp_ratio, advantages * tf.clip_by_value(logp_ratio, 1 - clip_param, 1 + clip_param)
        )
        ...
        loss = reduce_mean_valid(
            -surrogate_loss + cur_kl_coeff * action_kl + vf_loss_coeff * vf_loss - entropy_coeff * curr_entropy
        )
...
def kl_and_loss_stats(policy, train_batch):
    # TODO add mirror stuff and see if anything breaks
    return {
...
def vf_preds_and_logits_fetches(policy):
    ...
    return {SampleBatch.VF_PREDS: policy.model.value_function(), BEHAVIOUR_LOGITS: policy.model.last_output()}
