...
# C * sin(w * t).
class CGWalkerPlant(VectorSystem):
    def __init__(self, m=3, l=1, g=10, b=2, C=2, w=2):
        VectorSystem.__init__(
            self, 1, 3  # Two inputs (torque at shoulder).
        )  # Three outputs (theta, dtheta, base positions)
    ...
    def _DoCalcVectorTimeDerivatives(self, context, u, x, xdot):
        ...
        if abs(u) > 1e3:
            raise ValueError(
                "Input torque was excessive and would lead"
...
class PendulumController(VectorSystem):
    ...
    def __init__(self, feedback_rule):
        VectorSystem.__init__(
            self, 3, 1  # Three inputs (theta, dtheta, base position).
