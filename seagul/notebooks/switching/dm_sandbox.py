...
def do_rollout(x):
    th1, th2, dth1, dth2= x
    obs = env.reset(init_vec=np.array([th1, th2, dth1, dth2]))
    reward_hist = []
    obs_hist = []
    for i in range(env.num_steps):
        #actions = np.clip(np.asarray(control(env.dm_env.physics.state())), -max_torque, max_torque)
        actions = 0
        obs, reward, done, _ = env.step(actions)
        reward_hist.append(np.copy(reward))
        obs_hist.append(env.dm_env.physics.state())

    return reward_hist, obs_hist
