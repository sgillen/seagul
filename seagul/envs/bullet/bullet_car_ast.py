...
class RacecarGymEnvAst_v1(gym.Env):
    metadata = {
    ...
    def __init__(self,
                             ...
                             isDiscrete=False,
                             renders=False): #just change to True if you want to visualize
        print("init")
        self._timeStep = 0.01
    ...
    def get_real_obs(self):
        ...
        linkWorldPosition, linkWorldOrientation, localInertialFramePosition, localInertialFrameOrientation, \
        worldLinkFramePosition, worldLinkFrameOrientation, worldLinkLinearVelocity, worldLinkAngularVelocity = self._p.getLinkState(self._racecar.racecarUniqueId, 20, computeLinkVelocity=True)
    ...
    def step(self, action):
        ...
        for i in range(self._actionRepeat):
            ...
            if self.useSphere:
                ...
                if self._termination() or (np.sqrt(np.sum(np.array(self._observation)**2)) < self.dist_to_goal_threshold):
                    break
        ...
        if self.useSphere:
            ...
            done = self._termination() or (np.sqrt(np.sum(np.array(self._observation)**2)) < self.dist_to_goal_threshold)
