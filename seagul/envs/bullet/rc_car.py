...
class RcCar:
    ...
    def reset(self):
        ...
        car = self._p.loadURDF(self.urdfRootPath + "/racecar_differential_me.urdf" , [0, 0, .05],
                               useFixedBase=False)
    ...
    def get_w0_state(self):
        ...
        w0 = np.array([pos[0], pos[1], yaw, np.mean(steering_jointPositions), linVel[0], linVel[1], angVel[2],
                       np.mean(steering_jointVelocities), 0., 0.])
        return w0
    ...
    def applyAction(self, motorCommands):
        ...
        for steer in self.steeringLinks:
            self._p.setJointMotorControl2(self.racecarUniqueId, steer, self._p.POSITION_CONTROL,
                                          targetPosition=steeringAngle, force=self.maxForce)
