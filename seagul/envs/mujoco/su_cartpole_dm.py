...
@SUITE.add("benchmarking")
def balance_sparse(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
...
@SUITE.add()
def three_poles(time_limit=_DEFAULT_TIME_LIMIT, random=None, num_poles=3, sparse=False, environment_kwargs=None):
...
def _make_model(n_poles):
    ...
    for pole_index in range(2, n_poles + 1):
        child = etree.Element("body", name="pole_{}".format(pole_index), pos="0 0 1", childclass="pole")
...
class Physics(mujoco.Physics):
    ...
    def bounded_position(self):
        ...
        return np.hstack((self.cart_position(), self.named.data.xmat[2:, ["zz", "xz"]].ravel()))
...
class Balance(base.Task):
  ...
  """
    ...
    def _get_reward(self, physics, sparse):
        if sparse:
            cart_in_bounds = rewards.tolerance(physics.cart_position(), self._CART_RANGE)
            angle_in_bounds = rewards.tolerance(physics.pole_angle_cosine(), self._ANGLE_COSINE_RANGE).prod()
            ...
            small_control = rewards.tolerance(physics.control(), margin=1, value_at_margin=0, sigmoid="quadratic")[0]
