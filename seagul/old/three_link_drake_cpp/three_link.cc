#include "drake/examples/three_link/three_link.h"
...
template <typename T>
ThreeLink<T>::ThreeLink()
    : systems::LeafSystem<T>(
          systems::SystemTypeTag<examples::compass_gait::ThreeLink>{}) {
  this->DeclareContinuousState(ThreeLinkContinuousState<T>(), 2, 2, 0);
  ...
  this->DeclareVectorOutputPort(ThreeLinkContinuousState<T>(),
                                &ThreeLink::MinimalStateOut);

  // The floating-base (RPY) state of the system (useful for visualization).
  this->DeclareVectorOutputPort(systems::BasicVector<T>(14),
                                &ThreeLink::FloatingBaseStateOut);

  this->DeclareNumericParameter(ThreeLinkParams<T>());

  // Create the witness function.
  foot_collision_ = this->DeclareWitnessFunction(
      "foot collision",
      systems::WitnessFunctionDirection::kPositiveThenNonPositive,
      &ThreeLink::FootCollision, &ThreeLink::CollisionDynamics);
}

template <typename T>
T ThreeLink<T>::DoCalcKineticEnergy(
    const systems::Context<T>& context) const {
  const ThreeLinkContinuousState<T>& cg_state = get_continuous_state(context);
  const ThreeLinkParams<T>& params = get_parameters(context);
...
template <typename T>
T ThreeLink<T>::DoCalcPotentialEnergy(
    const systems::Context<T>& context) const {
  const ThreeLinkContinuousState<T>& cg_state = get_continuous_state(context);
  const ThreeLinkParams<T>& params = get_parameters(context);
...
template <typename T>
T ThreeLink<T>::FootCollision(const systems::Context<T>& context) const {
  const ThreeLinkContinuousState<T>& cg_state = get_continuous_state(context);
  const ThreeLinkParams<T>& params = get_parameters(context);
...
template <typename T>
void ThreeLink<T>::CollisionDynamics(
    const systems::Context<T>& context,
    const systems::UnrestrictedUpdateEvent<T>&,
    systems::State<T>* state) const {
  const ThreeLinkContinuousState<T>& cg_state = get_continuous_state(context);
  ThreeLinkContinuousState<T>& next_state =
...
template <typename T>
void ThreeLink<T>::MinimalStateOut(
    const systems::Context<T>& context,
...
template <typename T>
void ThreeLink<T>::FloatingBaseStateOut(
...
template <typename T>
Vector2<T> ThreeLink<T>::DynamicsBiasTerm(
    const systems::Context<T>& context) const {
  const ThreeLinkContinuousState<T>& cg_state = get_continuous_state(context);
  const ThreeLinkParams<T>& params = get_parameters(context);
...
template <typename T>
Matrix2<T> ThreeLink<T>::MassMatrix(
    const systems::Context<T>& context) const {
  const ThreeLinkContinuousState<T>& cg_state = get_continuous_state(context);
  const ThreeLinkParams<T>& params = get_parameters(context);
...
template <typename T>
void ThreeLink<T>::DoCalcTimeDerivatives(
    const systems::Context<T>& context,
...
template <typename T>
void ThreeLink<T>::DoGetWitnessFunctions(
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(
    class ::drake::examples::compass_gait::ThreeLink)
